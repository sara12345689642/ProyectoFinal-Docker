...
class Connection {
    ...
    // Propiedad privada que almacena la contraseña del usuario de la base de datos, obtenida del entorno
    private $password = '';

    // Método público que establece la conexión con la base de datos y devuelve un objeto PDO
    public function connect() {
        try {
            // Obtener la contraseña del entorno si está definida, de lo contrario usar una por defecto
            $this->password = getenv('MYSQL_ROOT_PASSWORD') ?: '';

            // Construcción del Data Source Name (DSN), que especifica la fuente de datos para PDO
